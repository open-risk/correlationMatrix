...
print("> Loading multi-period correlational matrices (cumulative mode) from json file")
SnP_Set0 = cm.CorrelationMatrixSet(json_file=dataset_path + "sp_NR_adjusted.json", temporal_type='Cumulative')
print("> Validate")
...
SnP = cm.CorrelationMatrixSet(dimension=8, periods=timesteps)
...
for k in t_list:
    ...
    if i < len(t_list) - 1:
        ...
        if gap > 1:
            # Divide right matrix by left matrix to derive forward gap correlation matrix the for gap-periods
            lm = SnP_Set0.entries[i]
            lm.fix_rowsums()
            rm = SnP_Set0.entries[i + 1]
            rm.fix_rowsums()
            # TODO Fix Negative probabilities for gap correlation matrix
            q = rm * lm.I
            q.fix_rowsums()
            #   From gap correlation matrix derive gap one-year matrices (via generator)
...
# TODO Handle strictly zero correlation probabilities
# TODO Handle non-monotonic correlation probabilities
