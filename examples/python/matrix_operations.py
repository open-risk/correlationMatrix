...
B = cm.CorrelationMatrix(dimension=4)
print(B)

print("> Any list can be used for initialization (but not all shapes are valid correlation matrices!)")
C = cm.CorrelationMatrix(values=[1.0, 3.0])
print(C)
...
F = cm.CorrelationMatrix(json_file=dataset_path + "JLT.json")
print(F)
...
E = cm.CorrelationMatrix(values=[[0.75, 0.25], [0.0, 1.0]])
print(E.validate())
# ATTRIBUTES
...
C = cm.CorrelationMatrix(values=np.resize(C, (2, 2)))
C[0, 1] = 0.0
C[1, 0] = 0.0
C[1, 1] = 1.0
print(C.validate())

print("> Computing the generator of a correlation matrix")
# Generator of A
G = A.generator()
print(A, expm(G))

print("> correlation Matrix algebra is very intuitive")
print(A * A)
print(A ** 2)
print(A ** 10)

print("> correlation matrices properties can be analyzed")
...
E = cm.CorrelationMatrix(values=JLT)
E_2 = cm.CorrelationMatrix(json_file=dataset_path + "JLT.json")
E_3 = cm.CorrelationMatrix(csv_file=dataset_path + "JLT.csv")
